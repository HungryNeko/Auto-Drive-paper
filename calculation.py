...
LastEditors: Andy
LastEditTime: 2024-01-23 13:17:21
'''
...
def calculate(id):
    ...
    x1 = y1 = x2 = y2 = t1 = t2 = v1 = v2 = 0

    for data_entry in sorted_data:
        x1 = x2; y1 = y2; t1 = t2
        x2 = data_entry['longitude']; y2 = data_entry['latitude']; t2 = data_entry['time']    
        if x1 == 0 or not carIsIn5(x2, y2): continue
        v1 = v2
        dt = t2 - t1
        if dt.total_seconds() == 0: continue
        distance = distanceCal(x1, y1, x2, y2)
        v2 = distance / dt.total_seconds()
        if v1 == 0 or distance == 0: continue
        acceleration = (v2 - v1) / dt.total_seconds()
        ...
        for key, values in car_result.items():
            for entry in values:
                if all(entry.get(field) is not None for field in ['time', 'speed', 'acceleration', 'angle', 'lon', 'lat']):
                    if entry['speed'] >= 34 or entry['speed'] <= 0:
